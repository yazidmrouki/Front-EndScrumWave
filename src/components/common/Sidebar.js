...
function Sidebar(props) {
    ...
    useEffect(() => {
        ...
        if (userType === 'ProductOwners') {
            const filteredMenu = menu.menu.filter(parent => parent.identifier !== 'Our Clients' && parent.identifier !== 'Accounts' && parent.identifier !== 'uiComponents' && parent.identifier !== 'Other Pages' )
                                           .map(parent => {
                                               if (parent.children) {
                                                   const filteredChildren = parent.children.filter(child => {
                                                       return child.identifier !== 'ProjectManagement' && 
                                                              child.identifier !== 'Team' && 
                                                              child.identifier !== 'Tasks' && 
                                                              child.identifier !== 'Timesheet'&&
                                                              child.identifier !== 'Invoices' && 
                                                              child.identifier !== 'Expenses' && 
                                                              child.identifier !== 'Department'&&
                                                              child.identifier !== 'Leave Request' && 
                                                              child.identifier !== 'Tickets Detail';
                                                   });
        ...
        } else if (userType === 'Devellopeurs' ) {
            const filteredMenu = menu.menu.filter(parent => {
                return parent.children && parent.children.some(child => {
                    return child.identifier === 'ProjectManagement' || child.identifier === 'Members Profile'|| child.identifier === 'Calander' || child.identifier === 'Chat App';
                });
            }).map(parent => {
                const filteredChildren = parent.children.filter(child => {
                    return child.identifier === 'ProjectManagement' || child.identifier === 'Members Profile' || child.identifier === 'Team'|| child.identifier === 'Calander' || child.identifier === 'Chat App';
                });
                return { ...parent, children: filteredChildren };
            });
            setMenuData(filteredMenu);
        }
        else if (userType === 'scrumMasters' ) {
            const filteredMenu = menu.menu.filter(parent => {
                return parent.children && parent.children.some(child => {
                    return child.identifier === 'Tasks' || child.identifier === 'Members Profile'|| child.identifier === 'Calander' || child.identifier === 'Tickets Detail' ||child.identifier === 'Chat App'|| child.identifier === 'Expenses';
                });
            }).map(parent => {
                const filteredChildren = parent.children.filter(child => {
                    return child.identifier === 'Tasks' || child.identifier === 'Members Profile' || child.identifier === 'Team'|| child.identifier === 'Tickets Detail' || child.identifier === 'Calander' || child.identifier === 'Chat App'|| child.identifier === 'Expenses';
                });
                return { ...parent, children: filteredChildren };
            });
            setMenuData(filteredMenu);
    ...
    return (
        ...
        <div className="d-flex flex-column h-100">
            <a href="hr-dashboard" className="mb-0 brand-icon">
                ...
                <span className="logo-text">Scrum-Wave</span>
            </a>
